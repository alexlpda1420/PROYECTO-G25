...
- entrenamiento de un modelo de Machine Learning supervisado de regresión (RandomForest)
...
- Requisitos: pandas, numpy, scikit-learn, joblib, streamlit, altair
...
def train_and_predict(X, y):
    """
    Entrena un modelo de regresión supervisada (RandomForestRegressor)
    y devuelve el modelo entrenado y las predicciones para todos los productos.
    """
    # División en train / test para evaluar el modelo
    ...
    # Modelo de regresión supervisada basado en Random Forest
    ...
    # Predicciones para todos los productos (dataset completo) para armar el ranking
...
def pipeline():
    ...
    # Crear dataset supervisado para ML (regresión)
    X, y, feat_cols, target_col = create_supervised_dataset(
        pivot,
        n_lags=PAST_MONTHS_FEATURES,
    )
    print(f"Dataset supervisado: {X.shape[0]} productos, features: {list(X.columns)} -> target: {target_col}")

    # Mensaje explícito de diseño conceptual del modelo (Sprint 3)
    print("\n=== Diseño conceptual del modelo de Machine Learning ===")
    print("Tipo de problema   : regresión supervisada")
    print("Objetivo           : predecir la cantidad vendida de cada producto en el próximo mes")
    print(f"Entradas (X)       : ventas mensuales históricas de los últimos {PAST_MONTHS_FEATURES} meses por producto")
    print(f"Salida (y)         : ventas del mes siguiente ({target_col})")
    print("Algoritmo          : RandomForestRegressor (bosque de árboles de decisión)")

    # Entrenamiento, evaluación y predicciones
